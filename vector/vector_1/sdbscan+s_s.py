...
# === 2. Standard Scaling (important for distance-based clustering) ===
vectors_scaled = StandardScaler().fit_transform(vectors)

# === 3. Shared Nearest Neighbor Matrix (SNN) 계산 ===
k = 10  # number of nearest neighbors
nbrs = NearestNeighbors(n_neighbors=k, metric="euclidean").fit(vectors_scaled)
_, knn_indices = nbrs.kneighbors(vectors_scaled)
...
# === 4. DBSCAN with precomputed distance ===
dbscan = DBSCAN(eps=4.5, min_samples=6, metric="precomputed")
...
# === 6. Copy associated text files into cluster folders ===
...
# === 7. Plotting with cluster labels ===
...
# === 8. Top 20 distant points by PC1 ===
